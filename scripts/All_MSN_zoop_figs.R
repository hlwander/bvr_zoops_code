...
zoop <- zoop[substrEnd(zoop$site_no,6)!="schind" & zoop$site_no!="BVR_d" & 
               zoop$site_no!="BVR_dam" & zoop$site_no!="BVR_trap" & 
               zoop$site_no!="FCR_50",]
...
zoop.repmeans <- zoop %>% select(sample_ID,site_no,collect_date,Hour, 
                                 ...
                                 Cladocera_density_NopL, 
                                 Cladocera_BiomassConcentration_ugpL, 
                                 Cladocera_PercentOfTotal,
                                 Cyclopoida_density_NopL, 
                                 Cyclopoida_BiomassConcentration_ugpL, 
                                 Cyclopoida_PercentOfTotal,
                                 Rotifera_density_NopL, 
                                 Rotifera_BiomassConcentration_ugpL,
                                 Rotifera_PercentOfTotal,
                                 Calanoida_PercentOfTotal, 
                                 Calanoida_density_NopL, 
                                 Calanoida_BiomassConcentration_ugpL,
                                 Copepoda_PercentOfTotal, 
                                 Copepoda_density_NopL, 
                                 Copepoda_BiomassConcentration_ugpL,
                                 nauplius_PercentOfTotal, 
                                 nauplius_density_NopL, 
                                 nauplius_BiomassConcentration_ugpL) %>%
  group_by(sample_ID, site_no, Hour, collect_date) %>%
  summarise_at(vars(ZoopDensity_No.pL:nauplius_BiomassConcentration_ugpL), 
               list(rep.mean=mean, rep.SE=stderr))

#merge collect_date and hour in a new column
zoop.repmeans$datetime<- paste(zoop.repmeans$collect_date,zoop.repmeans$Hour,sep=" ")
#get times into date format (character here)
zoop.repmeans$datetime<- format(as.POSIXct(zoop.repmeans$datetime,
                                           format="%Y-%m-%d %H:%M"), 
                                format="%Y-%m-%d %H:%M:%S")
#convert to posixct date format
zoop.repmeans$datetime<- as.POSIXct(zoop.repmeans$datetime, 
                                    format="%Y-%m-%d %H:%M")
...
zoop.repmeans$Hour <- strptime(paste0(as.character(zoop.repmeans$dates), 
...
df1 <- zoop_DHM |>  
  gather(metric,value,ZoopDensity_No.pL_rep.mean:nauplius_BiomassConcentration_ugpL_rep.mean)
df2 <- zoop_DHM |> 
  gather(metric.SE,value.SE, ZoopDensity_No.pL_rep.SE:nauplius_BiomassConcentration_ugpL_rep.SE)
...
variables <- c("ZoopDensity_No.pL_rep.mean","Cladocera_density_NopL_rep.mean", 
               "Cladocera_PercentOfTotal_rep.mean",
               "Cyclopoida_density_NopL_rep.mean",
               "Cyclopoida_PercentOfTotal_rep.mean","Rotifera_density_NopL_rep.mean",
               "Rotifera_PercentOfTotal_rep.mean","Calanoida_density_NopL_rep.mean",
               "Calanoida_PercentOfTotal_rep.mean",
...
SE <- c("ZoopDensity_No.pL_rep.SE","Cladocera_density_NopL_rep.SE", 
        "Cladocera_PercentOfTotal_rep.SE",
        "Cyclopoida_density_NopL_rep.SE","Cyclopoida_PercentOfTotal_rep.SE",
        "Rotifera_density_NopL_rep.SE", "Rotifera_PercentOfTotal_rep.SE"
        ,"Calanoida_density_NopL_rep.SE","Calanoida_PercentOfTotal_rep.SE",
...
#add column for MSN #
zoop_DHM_long$MSN <- ifelse(zoop_DHM_long$collect_date=="2019-07-10" | 
                            zoop_DHM_long$collect_date=="2019-07-11",1,
                     ifelse(zoop_DHM_long$collect_date=="2019-07-24" | 
                            zoop_DHM_long$collect_date=="2019-07-25",2,
                     ifelse(zoop_DHM_long$collect_date=="2020-08-12" | 
                            zoop_DHM_long$collect_date=="2020-08-13",3,
                     ifelse(zoop_DHM_long$collect_date=="2021-06-15" | 
                            zoop_DHM_long$collect_date=="2021-06-16",4,5))))
...
ggplot(subset(zoop_DHM_long, metric %in% c("Cladocera_density_NopL",
              "Copepoda_density_NopL","Rotifera_density_NopL")),
...
zoop_dens_stand <- data.frame(subset(zoop_DHM_long, 
                          metric %in% c("Cladocera_density_NopL",
                                        "Copepoda_density_NopL",
                                        "Rotifera_density_NopL")))
...
mean(zoop_dens_stand$value_max_std[
  zoop_dens_stand$metric=="Cladocera_density_NopL" & 
    zoop_dens_stand$site_no=="BVR_50"])
mean(zoop_dens_stand$value_max_std[
  zoop_dens_stand$metric=="Copepoda_density_NopL" & 
    zoop_dens_stand$site_no=="BVR_50"])
mean(zoop_dens_stand$value_max_std[
  zoop_dens_stand$metric=="Rotifera_density_NopL" & 
    zoop_dens_stand$site_no=="BVR_50"])

mean(zoop_dens_stand$value_max_std[
  zoop_dens_stand$metric=="Cladocera_density_NopL" & 
    zoop_dens_stand$site_no=="BVR_l"])
mean(zoop_dens_stand$value_max_std[
  zoop_dens_stand$metric=="Copepoda_density_NopL" & 
    zoop_dens_stand$site_no=="BVR_l"])
mean(zoop_dens_stand$value_max_std[
...
zoop_size <- zoop %>% select(sample_ID,site_no,collect_date,Hour, 
                             which(grepl("MeanSize",colnames(zoop)))) %>%
  group_by(sample_ID, site_no, Hour, collect_date) %>%
  summarise_at(vars(OverallMeanSize_mm:LecaneMeanSize_mm), 
               list(rep.mean=mean, rep.SE=stderr))

#merge collect_date and hour in a new column
zoop_size$datetime<- paste(zoop_size$collect_date,zoop_size$Hour,sep=" ")
#get times into date format (character here)
zoop_size$datetime<- format(as.POSIXct(zoop_size$datetime,format="%Y-%m-%d %H:%M"), 
                            format="%Y-%m-%d %H:%M:%S")
#convert to posixct date format
zoop_size$datetime<- as.POSIXct(zoop_size$datetime, format="%Y-%m-%d %H:%M")

zoop_size$dates <- ifelse(zoop_size$collect_date=="2019-07-10" | 
                            zoop_size$collect_date=="2019-07-24"| 
                            zoop_size$collect_date=="2020-08-12" | 
                            zoop_size$collect_date=="2021-06-15" |
                            zoop_size$collect_date=="2021-07-07",
                          "2022-10-15","2022-10-16")
...
#combine hour and date
zoop_size$Hour <- strptime(paste0(as.character(zoop_size$dates), 
                                  zoop_size$Hour),format="%Y-%m-%d %H:%M")
...
#only select epi samples for DHM plots
zoop_size_epi <- zoop_size[grepl("epi",zoop_size$sample_ID) 
                           |grepl("sunrise",zoop_size$sample_ID) | 
                             grepl("sunset",zoop_size$sample_ID) | 
                             zoop_size$site_no=="BVR_l",]

#convert new dfs from tibble to dataframe 
zoop_size_DHM <- data.frame(zoop_size_epi)

#convert df from wide to long (kinda hacky way bc having problems doing this)
df1_size <- zoop_size_DHM |>  
  gather(metric,value,OverallMeanSize_mm_rep.mean:LecaneMeanSize_mm_rep.mean)
df2_size <- zoop_size_DHM |>  
  gather(metric.SE,value.SE, OverallMeanSize_mm_rep.SE:LecaneMeanSize_mm_rep.SE)
...
#drop _rep.mean from all metric names
zoop_size_DHM_long$metric <- 
  substr(zoop_size_DHM_long$metric,1,nchar(zoop_size_DHM_long$metric)-9)

#add column for MSN #
zoop_size_DHM_long$MSN <- ifelse(zoop_size_DHM_long$collect_date=="2019-07-10" | 
                                   zoop_size_DHM_long$collect_date=="2019-07-11",1,
                          ifelse(zoop_size_DHM_long$collect_date=="2019-07-24" | 
                                   zoop_size_DHM_long$collect_date=="2019-07-25",2,
                          ifelse(zoop_size_DHM_long$collect_date=="2020-08-12" | 
                                   zoop_size_DHM_long$collect_date=="2020-08-13",3,
                          ifelse(zoop_size_DHM_long$collect_date=="2021-06-15" | 
                                  zoop_size_DHM_long$collect_date=="2021-06-16",4,5))))
...
#Figure S4
ggplot(subset(zoop_size_DHM_long, metric %in% c("CladoceraMeanSize_mm",
                                                "CopepodaMeanSize_mm",
                                                "RotiferaMeanSize_mm")),
       aes(Hour,value, color=as.factor(MSN))) + 
