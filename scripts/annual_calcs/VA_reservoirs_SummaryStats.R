#### Code for Zooplankton Summary Statistics - Density and Biomass calculations
### Code modified from David C. Richardson (DCR)
### Generates summary csv files to use for figure generation
...
ZoopTaxonomy<-read.csv("conversions/Zooplankton-TaxonomicIdentification.csv", 
                       fill = TRUE,as.is=TRUE)

#record which rows are NA
SpeciesNA.rows<-which(is.na(ZoopTaxonomy$Species)) 
#Replace the species with first letter of Genus and Species, e.g., D. magna
...
#Replace the species with first letter of Genus and Species, e.g., K. earlinae
RotiferBiomassConversions$Species[!is.na(RotiferBiomassConversions$Species)]<-
  paste0(substring(RotiferBiomassConversions$Genus[
    !is.na(RotiferBiomassConversions$Species)],1,1),". ",
    RotiferBiomassConversions$Species[!is.na(RotiferBiomassConversions$Species)])

#Read in csv file for size to biomass conversions for crustaceans with all taxa ID'ed
CrustaceanBiomassConversions<-read.csv("conversions/ZooplanktonLengthBiomassConversion-Crustaceans.csv", 
                                       fill = TRUE,as.is=TRUE, na.strings = "") 

#Replace the species with first letter of Genus and Species, e.g., D. magna
CrustaceanBiomassConversions$Species[!is.na(CrustaceanBiomassConversions$Species)]<-
  paste0(substring(CrustaceanBiomassConversions$Genus[
    !is.na(CrustaceanBiomassConversions$Species)],1,1),". ",
    CrustaceanBiomassConversions$Species[!is.na(CrustaceanBiomassConversions$Species)])

#Move all Cladacora ids to suborder
CrustaceanBiomassConversions$Suborder[
  CrustaceanBiomassConversions$Order=="Cladocera"]<-"Cladocera"

CrustaceanBiomassConversions$Order[
  CrustaceanBiomassConversions$Order=="Cladocera"]<-""

#Make sure all Daphniidae are subOrder cladocera
CrustaceanBiomassConversions$Suborder[
  CrustaceanBiomassConversions$Family=="Daphniidae"]<-"Cladocera"
...
for(y in 1:length(year)){
  ...
  for(j in 1:length(SizeID$TaxaID)){
    ...
    for(k in 1:TaxaColumn){
        ...
        for(rot.i in 5:10){
        if(SizeID$TaxaID[j]=="Cyclopoida" | SizeID$TaxaID[j]=="Calanoida" & 
           is.na(SizeID$Nauplius[j]=="nauplius")){
  ...
  SizeID$Biomass_ug[SizeID$Genus=="Asplanchna"& !is.na(SizeID$Genus)] <-  
    SizeID$Biomass_ug[SizeID$Genus=="Asplanchna"& !is.na(SizeID$Genus)]*0.39
  ...
  head(SizeID[order(SizeID$Biomass_ug,decreasing=TRUE),
              c("LowestTaxonomicLevelOfID","TaxaID","Size_mm","Biomass_ug")],15)
  temp<-SizeID[order(SizeID$Biomass_ug,decreasing=TRUE),]
  head(temp[temp$Phylum=="Rotifera"&!is.na(temp$Phylum),
            c("LowestTaxonomicLevelOfID","TaxaID","Size_mm","Biomass_ug")],15)
  head(temp[temp$Subphylum=="Crustacea"&!is.na(temp$Phylum),
            c("LowestTaxonomicLevelOfID","TaxaID","Size_mm","Biomass_ug")],15)
  median(temp[temp$Subphylum=="Crustacea"&!is.na(temp$Phylum),
              c("Biomass_ug")],na.rm=T)
  length(temp[temp$Subphylum=="Crustacea"&!is.na(temp$Phylum),
              c("Biomass_ug")])
  length(temp[temp$Phylum=="Rotifera"&!is.na(temp$Phylum),
              c("Biomass_ug")])
  ...
  ZoopFinal<- merge(ZoopFinal,setNames(aggregate(SizeID$Size_mm~SizeID$sample_ID,
                                                 FUN=mean,na.rm=TRUE),
                                       c("sample_ID","OverallMeanSize_mm")),
                                       by=c("sample_ID"),all.x=T)
  #SD of size by site_ID
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Size_mm~sample_ID,data=SizeID,
                                                FUN=sd,na.action=na.omit),
                                      c("sample_ID","OverallSESize_mm")),
                                      by=c("sample_ID"),all.x=T)
  #count of size by site_ID
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(TaxaID~sample_ID,data=SizeID,
                                                FUN=length,na.action=na.pass),
                                      c("sample_ID","OverallCount_n")),
                                      by=c("sample_ID"),all.x=T)
  ...
  for(taxa.index in 1:length(taxaOfInterest)){
    ...
    if(nrow(temp)==0){
      ZoopFinal[,c(paste0(taxa,"MeanSize_mm"),paste0(taxa,"SESize_mm"))]<-NA
      ZoopFinal[,c(paste0(taxa,"Count_n"),paste0(taxa,"_PercentOfTotal"),
                   paste0(taxa,"_totalbiomass_ug"),paste0(
                     taxa,"_PercentOftotalbiomassConcentration"))]<-0
    }else{
      #Average size by sample_ID
      ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Size_mm~sample_ID,data=temp,
                                                    FUN=mean,na.action=na.omit),
                                          c("sample_ID",paste0(taxa,"MeanSize_mm"))),
                                          by=c("sample_ID"),all.x=T)
      #SD of size by sample_ID
      ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Size_mm~sample_ID,
                                                    data=temp,FUN=sd,na.action=na.omit),
                                          c("sample_ID",paste0(taxa,"SESize_mm"))),
                                          by=c("sample_ID"),all.x=T)
      #Count by sample_ID
      ZoopFinal<-merge(ZoopFinal,setNames(aggregate(TaxaID~sample_ID,
                                                    data=temp,FUN=length),
                                          c("sample_ID",paste0(taxa,"Count_n"))),
                                          by=c("sample_ID"),all.x=T)
      #Percent that are that taxa
      ZoopFinal[,paste0(taxa,"_PercentOfTotal")] <- ZoopFinal[,paste0(taxa,"Count_n")] * 100 / 
                                                    ZoopFinal$OverallCount_n
      ...
      ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Biomass_ug~sample_ID,data=temp,
                                                    FUN=sum,na.action=na.omit),
                                          c("sample_ID",paste0(taxa,"_biomass_unadj"))),
                                          by=c("sample_ID"),all.x=T)
      #this is number of individuals (within each taxa) measured per sample
      ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Size_mm~sample_ID,data=temp,FUN=length),
                                          c("sample_ID",paste0(taxa,"_zoops_measured"))),
                                          by=c("sample_ID"),all.x=T)
      #now calculate total biomass by multiplying biom_unadj by count/zoops_measured
      ZoopFinal[,paste0(taxa,"_totalbiomass_ug")]<- 
        ZoopFinal[,paste0(taxa,"_biomass_unadj")] * (ZoopFinal[,paste0(taxa,"Count_n")] / 
                                                       ZoopFinal[,paste0(taxa,"_zoops_measured")])
     
      #Taxa density
     ZoopFinal[,paste(taxa,"_density_NopL",sep="")]<-
       ifelse(ZoopFinal$site_no=="BVR_trap" | ZoopFinal$site_no=="BVR_schind" | 
                ZoopFinal$site_no=="FCR_schind", 
                ZoopFinal[,paste0(taxa,"_density_NopL")] <- ZoopFinal[,paste0(taxa,"Count_n")] / 
                ZoopFinal$Volume_unadj, 
       ifelse(substr(ZoopFinal$site_no,1,3)=="BVR" & ZoopFinal$DepthOfTow_m > 8, 
                ZoopFinal[,paste(taxa,"_density_NopL",sep="")] <- 
                ZoopFinal[,paste(taxa,"Count_n",sep="")] * (1/bvr_neteff)/ 
                ZoopFinal$Volume_L, 
       ifelse(substr(ZoopFinal$site_no,1,3)=="FCR" & ZoopFinal$DepthOfTow_m > 8, 
                ZoopFinal[,paste(taxa,"_density_NopL",sep="")] <- 
                ZoopFinal[,paste(taxa,"Count_n",sep="")]* (1/fcr_neteff)/ 
                ZoopFinal$Volume_L, ZoopFinal[,paste0(taxa,"_density_NopL")] <- 
                ZoopFinal[,paste(taxa,"Count_n",sep="")] / ZoopFinal$Volume_L)))
      
      #Taxa biomass concentration
      ZoopFinal[,paste(taxa,"_BiomassConcentration_ugpL",sep="")]<-
        ifelse(ZoopFinal$site_no=="BVR_trap" | ZoopFinal$site_no=="BVR_schind" | 
               ZoopFinal$site_no=="FCR_schind", 
               ZoopFinal[,paste0(taxa,"_BiomassConcentration_ugpL")]<- 
               ZoopFinal[,paste(taxa,"_totalbiomass_ug",sep="")] / ZoopFinal$Volume_unadj,
        ifelse(substr(ZoopFinal$site_no,1,3)=="BVR" & ZoopFinal$DepthOfTow_m > 8, 
               ZoopFinal[,paste(taxa,"_BiomassConcentration_ugpL",sep="")]<- 
               ZoopFinal[,paste(taxa,"_totalbiomass_ug",sep="")] * 
               (1/bvr_neteff)/ ZoopFinal$Volume_L,
        ifelse(substr(ZoopFinal$site_no,1,3)=="FCR" & 
               ZoopFinal$DepthOfTow_m > 8, 
               ZoopFinal[,paste0(taxa,"_BiomassConcentration_ugpL",sep="")]<- 
               ZoopFinal[,paste(taxa,"_totalbiomass_ug",sep="")] * (1/fcr_neteff)/ 
               ZoopFinal$Volume_L, ZoopFinal[,paste(taxa,"_BiomassConcentration_ugpL",sep="")]<- 
               ZoopFinal[,paste(taxa,"_totalbiomass_ug",sep="")] / ZoopFinal$Volume_L)))
      
      
      #Percent biomass concentration of total
      ZoopFinal[,paste(taxa,"_PercentOftotalbiomassConcentration",sep="")] <-
        ZoopFinal[,paste(taxa,"_BiomassConcentration_ugpL",sep="")] * 100 /
  ...
  ZoopFinal$TotalBiomass_ug<- rowSums(ZoopFinal[,paste0(TaxaforTotalCalcs,"_totalbiomass_ug")], 
                                      na.rm=TRUE)
  #Calculate biomass concentration as biomass relative to total volume counted
  ZoopFinal$BiomassConcentration_ugpL<-ifelse(ZoopFinal$site_no=="BVR_trap" | 
                                              ZoopFinal$site_no=="BVR_schind" | 
                                              ZoopFinal$site_no=="FCR_schind", 
                                              ZoopFinal$BiomassConcentration_ugpL <- 
                                              ZoopFinal$TotalBiomass_ug / ZoopFinal$Volume_unadj,
                                        ifelse(substr(ZoopFinal$site_no,1,3)=="BVR" & 
                                               ZoopFinal$DepthOfTow_m > 8, 
                                               ZoopFinal$BiomassConcentration_ugpL <- 
                                               ZoopFinal$TotalBiomass_ug *(1/bvr_neteff)/ 
                                               ZoopFinal$Volume_L,
                                         ifelse(substr(ZoopFinal$site_no,1,3)=="FCR" & 
                                               ZoopFinal$DepthOfTow_m > 8, 
                                               ZoopFinal$BiomassConcentration_ugpL <- 
                                               ZoopFinal$TotalBiomass_ug *(1/fcr_neteff)/ 
                                               ZoopFinal$Volume_L, 
                                               ZoopFinal$BiomassConcentration_ugpL <- 
  ...
  write.csv(ZoopFinal,paste0("output/FCR_ZooplanktonSummary",year[y],".csv"),row.names = FALSE)
...
if(sum(!is.na(SizeID[,c(taxa)]))>0){
  #Richness
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Genus~sample_ID,data=SizeID,FUN=function(x) 
    length(unique(x))),c("sample_ID",paste0(taxa,"Richness"))),by=c("sample_ID"),all.x=T)
  #ShannonDiversity
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Genus~sample_ID,data=SizeID,FUN=function(x) 
    -sum(prop.table(table(x[!is.na(x)]))*log(prop.table(table(x[!is.na(x)]))))),
    c("sample_ID",paste0(taxa,"ShannonDiversity"))),by=c("sample_ID"),all.x=T)
  #Evenness
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(Genus~sample_ID,data=SizeID,FUN=function(x) 
    -sum(prop.table(table(x[!is.na(x)]))*log(prop.table(table(x[!is.na(x)])))) /
      log(length(unique(x)))),c("sample_ID",paste0(taxa,"Evenness"))),by=c("sample_ID"),all.x=T)
  
}else{
  ZoopFinal[,c(paste0(taxa,"Richness"),paste0(taxa,"ShannonDiversity"),
               paste0(taxa,"Evenness"))]<-NA
}
...
SizeID.Order$OrderSubOrder<-apply(SizeID.Order[,c("Order","Suborder")], 1, 
                                  function(x) paste(na.omit(x),collapse=""))
SizeID.Order$OrderSubOrder[SizeID.Order$OrderSubOrder==""]<-NA
if(sum(!is.na(SizeID.Order[,c(taxa)]))>0){
  #Richness
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(OrderSubOrder~sample_ID,
                                       data=SizeID.Order,FUN=function(x) 
                                       length(unique(x))),c("sample_ID",
                                       paste0(taxa,"Richness"))),by=c("sample_ID"),all.x=T)
  #ShannonDiversity
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(OrderSubOrder~sample_ID,
                                       data=SizeID.Order,FUN=function(x) 
                                       -sum(prop.table(table(x[!is.na(x)])) * 
                                       log(prop.table(table(x[!is.na(x)]))))),
                                       c("sample_ID",paste0(taxa,"ShannonDiversity"))),
                                       by=c("sample_ID"),all.x=T)
  #Evenness
  ZoopFinal<-merge(ZoopFinal,setNames(aggregate(OrderSubOrder~sample_ID,data=SizeID.Order,
                                                FUN=function(x) -sum(prop.table(table(x[
                                                  !is.na(x)]))*log(prop.table(table(x[
                                                    !is.na(x)]))))/log(length(unique(x)))),
                                      c("sample_ID",paste0(taxa,"Evenness"))),
                                      by=c("sample_ID"),all.x=T)
