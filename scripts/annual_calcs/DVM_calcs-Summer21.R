...
zoop<- read.csv('output/FCR_ZooplanktonSummary2021.csv',header = TRUE)
...
zoop.repmeans <- zoop %>% select(sample_ID,site_no,collect_date,Hour, ZoopDensity_No.pL, BiomassConcentration_ugpL,
                  ...
                  summarise_at(vars(ZoopDensity_No.pL:nauplius_totalbiomass_ug,), list(rep.mean=mean, rep.SE=stderr)) #CHECK WARNING HERE!
